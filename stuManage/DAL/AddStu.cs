...
namespace stuManage.SQLServerDAL
{
	...
	/// 数据访问类:Stustay
	/// </summary>
	public partial class AddStu:IAddStu
	{
        public AddStu()
		{}
		#region  BasicMethod
		...
		public bool Exists(string num)
		{
            ...
            strSql.Append("select count(1) from AddStu");
		...
		{
			StringBuilder strSql=new StringBuilder();
            strSql.Append("insert into AddStu(");
		...
		{
			StringBuilder strSql=new StringBuilder();
            strSql.Append("update AddStu set ");
		...
		public bool Delete(string num)
		{
			...
			StringBuilder strSql=new StringBuilder();
            strSql.Append("delete from AddStu ");
			strSql.Append(" where num=@num ");
			SqlParameter[] parameters = {
					new SqlParameter("@num", SqlDbType.Char,10)			};
			parameters[0].Value = num;
		...
		/// 得到一个对象实体
		/// </summary>
        public stuManage.Model.AddStu DataRowToModel(DataRow row)
		{
            stuManage.Model.AddStu model = new stuManage.Model.AddStu();
			if (row != null)
			{
